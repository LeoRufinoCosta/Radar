...
def aviao(x, y, cor):
    # x = x1*math.cos(math.radians(ang))-y1*math.sin(math.radians(ang))
    # y = y1*math.cos(math.radians(ang))+x1*math.sin(math.radians(ang))
    c = Line(Point(x, y), Point((x - 10), y))
    ac = Line(Point((x - 3), y), Point((x - 4), (y - 4)))
    ab = Line(Point((x - 3), y), Point((x - 4), (y + 4)))
    rc = Line(Point((x - 8), y), Point((x - 9), (y - 2)))
    rb = Line(Point((x - 8), y), Point((x - 9), (y + 2)))
    t = Text(Point(x + 15, y - 5), (x, ",", y))
    t.setSize(6)
    t.setOutline(cor)
    c.setOutline(cor)
    ac.setOutline(cor)
...
def Tela_Fundo():
    ...
    while True:
        ...
        aviao(300, 250, "blue")
        aviao(450, 250, "red")
        aviao(580, 250, "purple")



while True:
    Texto(665, 300, "0º", "red", 10, "bold")
    Texto(400, 22, "90º", "red", 10, "bold")
    Texto(400, 560, "180º", "red", 10, "bold")
    Texto(140, 300, "270º", "red", 10, "bold")
